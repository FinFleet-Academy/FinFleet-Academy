...
const TradingDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (activeTab === 'portfolio' || activeTab === 'market') {
        ...
        await axios.post('/api/analytics/log-activity', { action: 'PAGE_VIEW', metadata: { page: 'trading_dashboard' } }, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        }).catch(() => {});
  ...
  const handleQuizSubmit = async (quizId, answers) => {
    try {
      ...
      toast.success(`Protocol Complete. Score: ${res.data.score}/${res.data.total}`);
      if (res.data.passed) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="py-32 text-center bg-[#F9FAFB] dark:bg-[#080C10] min-h-screen">
        ...
        <h2 className="text-3xl font-black dark:text-white mb-4 uppercase">Restricted Area</h2>
        <p className="text-slate-500 font-bold mb-8">Identity verification required for simulator access.</p>
        <button onClick={() => window.location.href='/login'} className="btn-brand py-4 px-10 text-[10px] uppercase tracking-widest">Authenticate</button>
      </div>
    );
  }

  const tabs = [
    { id: 'portfolio', name: 'Virtual Portfolio', icon: PieChart },
    { id: 'market', name: 'Trading Desk', icon: Activity },
    { id: 'sip', name: 'SIP Simulator', icon: Calculator },
    { id: 'quizzes', name: 'Intelligence Checks', icon: BrainCircuit },
    { id: 'leaderboard', name: 'Global Ranks', icon: Award }
  ];
      ...
      <div className="bg-amber-500/10 border-b border-amber-500/20 py-3 mb-10">
         <div className="max-w-7xl mx-auto px-4 flex items-center justify-center space-x-3">
            ...
            <p className="text-[10px] font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest text-center">
              Educational Simulator Active. No real capital involved. FinFleet does not provide financial advice.
            </p>
         </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
           <div>
              ...
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Institutional Simulation Protocol</p>
           </div>
           <div className="flex items-center space-x-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl shadow-sm">
              ...
              <div>
                 <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Fleet Rank Points</p>
            ...
            <motion.div key="portfolio" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-10">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 bg-slate-950 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl border border-slate-800">
                     ...
                     <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4">Total Virtual Net Worth</h3>
                     ...
                     <div className="grid grid-cols-2 gap-8 pt-8 border-t border-slate-800">
                        <div>
                           <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Available Capital</p>
                           <p className="text-2xl font-black text-white">₹{balance.toLocaleString('en-IN', { maximumFractionDigits: 2 })}</p>
                        </div>
                        <div>
                           <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Invested Value</p>
                  ...
                  <div className="bg-brand-50 dark:bg-brand-900/10 rounded-[3rem] p-10 border border-brand-100 dark:border-brand-800/50">
                     ...
                     <h3 className="text-lg font-black dark:text-white uppercase tracking-tighter mb-4">AI Market Logic</h3>
                     <p className="text-[11px] font-bold text-slate-600 dark:text-slate-400 leading-relaxed italic mb-6">
                       "Tech sector shows elevated volatility. Consider diversifying your simulated portfolio with index funds to stabilize virtual returns."
                     </p>
                     <button className="text-[9px] font-black text-brand-600 uppercase tracking-widest flex items-center hover:translate-x-1 transition-transform">
                        Full Analysis <ArrowRight className="w-3.5 h-3.5 ml-2" />
                     </button>
                  </div>
               </div>
               ...
               <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 border border-slate-200 dark:border-slate-800 shadow-sm">
                  <h3 className="text-[12px] font-black dark:text-white uppercase tracking-[0.2em] mb-8">Active Holdings</h3>
                           ...
                           {portfolio.length === 0 && (
                             <tr>
                               <td colSpan="5" className="py-12 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] italic">
                                 No assets acquired. Visit Trading Desk.
                               </td>
                             </tr>
                           )}
                        </tbody>
          ...
          {activeTab === 'market' && (
            <motion.div key="market" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
               <div className="lg:col-span-8 space-y-10">
                  <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 border border-slate-200 dark:border-slate-800 shadow-sm">
                     <h3 className="text-[12px] font-black dark:text-white uppercase tracking-[0.2em] mb-8">Market Feed</h3>
               ...
               <div className="lg:col-span-4">
                  <div className="bg-slate-950 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl sticky top-32">
                     ...
                     <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8">Execution Terminal</h3>
                     
                     <form onSubmit={handleTrade} className="space-y-6">
                        <div className="space-y-3">
                           <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Asset Node</label>
                        ...
                        <div className="pt-6 border-t border-white/10 flex justify-between items-center">
                           <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Est. Capital Required</span>
                        ...
                        <button type="submit" className="w-full bg-white text-slate-950 py-5 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all">Execute Order</button>
                     </form>
                  </div>
               </div>
            </motion.div>
          ...
          {activeTab === 'sip' && (
            <motion.div key="sip" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
               <div className="lg:col-span-5 bg-white dark:bg-slate-900 rounded-[3rem] p-10 border border-slate-200 dark:border-slate-800 shadow-sm">
                  <h3 className="text-[12px] font-black dark:text-white uppercase tracking-[0.2em] mb-8">SIP Configurator</h3>
                  <div className="space-y-8">
                     ...
                     <div className="space-y-4">
                        <div className="flex justify-between">
                           <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Expected Return Rate (p.a %)</label>
                     ...
                     <button onClick={calculateSIP} className="w-full btn-brand py-5 rounded-[2rem] text-[10px] font-black uppercase tracking-[0.2em]">Project Growth</button>
                  </div>
               </div>
               ...
               {sipResult && (
                 <div className="lg:col-span-7 bg-slate-950 rounded-[3rem] p-10 relative overflow-hidden shadow-2xl">
                    ...
                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-10">Projection Results</h3>
                    
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                       <div>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-2">Total Investment</p>
                          <p className="text-xl font-black text-white">₹{Math.round(sipResult.invested).toLocaleString()}</p>
                       </div>
                       <div>
                          <p className="text-[9px] font-bold text-emerald-400 uppercase tracking-widest mb-2">Est. Returns</p>
                     ...
                     <div key={quiz._id} className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group">
                        ...
                        <div className="flex justify-between items-center pt-6 border-t border-slate-100 dark:border-slate-800">
                           ...
                           <button onClick={() => {
                             ...
                             const answers = quiz.questions.map(() => 0); // guess all 0
                             handleQuizSubmit(quiz._id, answers);
                           }} className="px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[9px] font-black uppercase tracking-widest rounded-xl hover:scale-105 transition-all">
                             Start Protocol
                           </button>
                        </div>
                     </div>
          ...
          {activeTab === 'leaderboard' && (
            <motion.div key="leaderboard" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-4xl mx-auto">
               <div className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 border border-slate-200 dark:border-slate-800 shadow-xl">
                  <h3 className="text-[12px] font-black dark:text-white uppercase tracking-[0.2em] mb-10 text-center">Global Fleet Ranks</h3>
                  <div className="space-y-4">
                     {leaderboard.map((u, i) => (
                        <div key={u._id} className={`flex items-center justify-between p-6 rounded-[2rem] border transition-all ${i === 0 ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800/50' : i === 1 ? 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700' : 'bg-transparent border-transparent hover:bg-slate-50 dark:hover:bg-slate-900/50'}`}>
                           <div className="flex items-center space-x-6">
                              ...
                              <div>
                                 ...
                                 {i === 0 && <span className="text-[9px] font-black text-amber-500 uppercase tracking-widest mt-1">Fleet Commander</span>}
                              </div>
                           </div>
